...
export async function executeCodeExplorerTool(args: any): Promise<string> {
  try {
    ...
    if (!codeVectorizationService.isReady()) {
      ...
      console.log('[CODE-EXPLORER-TOOL-MAIN] Initializing code vectorization service for project:', projectPath);
      try {
        await codeVectorizationService.initialize(projectPath);
      } catch (initError) {
        console.error('[CODE-EXPLORER-TOOL-MAIN] Failed to initialize code vectorization service:', initError);
        return JSON.stringify({
          success: false,
